...
from pyvivado import interface, signal, builder, utils, config
...
def get_axi_interconnect_interface(params):
    ...
    wires_in = (
        ...
        ('i_m', axi_utils.axi4lite_m2s_type),
        ('i_s', signal.Array(contained_type=axi_utils.axi4lite_s2m_type,
                             size=len(slave_ids))),
    )
    wires_out = (
        ('o_m', axi_utils.axi4lite_s2m_type),
        ('o_s', signal.Array(contained_type=axi_utils.axi4lite_m2s_type,
                             size=len(slave_ids))),
    )
    packages = [
        'work.axi_utils',
